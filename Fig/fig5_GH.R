...
dose = read.xlsx('./P831_P845_P855药敏实验3X复孔.xlsx', sheet = 2)
dose20250828 = read.xlsx("./药敏试验20250828.xlsx", sheet = 2)

temp = dose
for (i in unique(temp$patient)) {
  temp[temp$patient == i, 2] = temp[temp$patient == i, 2]/temp[temp$patient == i & temp$drug == 'Ctrl', 2]
  temp[temp$patient == i, 3] = temp[temp$patient == i, 3]/temp[temp$patient == i & temp$drug == 'Ctrl', 3]
  temp[temp$patient == i, 4] = temp[temp$patient == i, 4]/temp[temp$patient == i & temp$drug == 'Ctrl', 4]
}
temp$temp = as.character(lapply(temp$drug, function(x){
  x = strsplit(x, ' ')[[1]]
  x[length(x)]
}))
temp$temp = gsub("[^0-9.]", "",  temp$temp)
temp = subset(temp, drug!='Ctrl')
temp$drug1 = rep(c("Cisplatin", "JNK-IN-7", "CAY10603", "THZ", "Citarinostat"), each = 6)
dose = temp
...
temp$temp = gsub("[^0-9.]", "",  temp$temp)
temp = subset(temp, drug!='ctrl')
temp$drug1 = c("JNK-IN-7", "JNK-IN-7",  "CAY10603", "CAY10603",  "JNK-IN-7", "JNK-IN-7", "Citarinostat", "Citarinostat" )
dose20250828 = temp

dose = rbind(dose, dose20250828)
dose = dose[order(dose$patient, dose$drug1, dose$temp),]
...
pid_cur = pid[3]
temp1 = dose[ dose$patient == pid_cur , ]
temp1$drug_dose = paste0(temp1$drug1, '_', temp1$temp)
temp1 = melt(temp1[c(2:4, 8)], id.vars = 'drug_dose')
head(temp1)
temp1$x = as.character(lapply(temp1$drug_dose, function(x){
  strsplit(x, '_')[[1]][2]
}))
temp1$drug1 = as.character(lapply(temp1$drug_dose, function(x){
  strsplit(x, '_')[[1]][1]
}))

temp1$x = as.numeric(temp1$x)
temp1$drugrep = paste0(temp1$drug1, '_', temp1$variable)
temp1$doseum = temp1$x
unique(temp1$doseum)
print(pid_cur)
...
############## 20260613 ##################
p_high <- ggplot(plotmat_bliss,aes(x = lx, y = y_1, color=drug))+
  ...
  geom_point(aes_(x = plotmat_bliss$lxmid, y = plotmat_bliss$ANCHOR_VIAB/2), shape = 2)#, color = "black"
p_high
plotlist[[pid_cur]] = p_high

ggsave("//172.16.13.136/share/members/chenghonghan/PTV1/proof_check20260608/fig5/P855_P831_20260614.pdf", plotlist[[1]]+plotlist[[2]], width = 12, height = 4)
